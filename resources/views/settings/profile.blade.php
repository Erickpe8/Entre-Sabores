...
@section('settings-sidebar-actions')
    <div class="flex flex-col items-center gap-3 text-center">
        <button
            type="button"
            id="openAvatarModal"
            class="inline-flex items-center gap-1.5 rounded-full border border-white/20 px-4 py-2 text-sm font-medium text-slate-200 transition hover:bg-white/10 hover:text-white"
        >
            <i data-lucide="image-plus" class="h-4 w-4 shrink-0" aria-hidden="true"></i>
            Cambiar foto
        </button>
...
@section('settings-content')
    <form method="post" action="{{ route('settings.profile.update') }}" class="space-y-6">
        ...
        @if (session('success') || session('status') === 'profile-updated')
            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2500)" class="text-sm text-center text-green-400">
                {{ session('success') ?? __('Guardado.') }}
            </p>
        @endif
    </form>
@endsection
    ...
    <script>
        (function () {
            ...
            async function validateUsername(raw) {
                ...
                if (normalized === lastChecked) return;
                if (raw.length < 3) return setStatus('Mínimo 3 caracteres.', 'text-amber-400/90');
                if (raw.length > 30) return setStatus('Máximo 30 caracteres.', 'text-red-400');
                if (!/^[a-z0-9_-]+$/.test(raw)) return setStatus('Solo minúsculas, números, guiones y guion bajo.', 'text-amber-400/90');
                ...
                try {
                    ...
                    setStatus(response.data?.available ? 'Disponible' : 'No disponible', response.data?.available ? 'text-green-400' : 'text-red-400');
                } catch (error) {
                    if (error?.name === 'CanceledError' || error?.code === 'ERR_CANCELED') return;
                    if (error?.response?.status === 422) return setStatus('Formato no válido.', 'text-red-400');
            ...
            input.addEventListener('input', () => {
                ...
                if (preview) preview.textContent = raw || originalUsername;
                clearTimeout(timer);
...
@endsection
<x-app-layout title="{{ __('Editar perfil') }} — {{ config('app.name') }}">
    <div class="min-h-[100dvh] bg-gradient-to-br from-[#020617] via-[#0f172a] to-[#022c22] pb-16">
        <div class="max-w-7xl mx-auto px-6 py-10 grid lg:grid-cols-3 gap-8">
            ...
            <main class="lg:col-span-2 space-y-6">
                @include('settings.partials.settings-nav', ['active' => 'profile'])
