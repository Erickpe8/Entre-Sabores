<x-app-layout title="{{ '@'.$user->username }} — {{ config('app.name') }}">
        ...
        <div class="max-w-7xl mx-auto px-6 py-10 grid lg:grid-cols-3 gap-8">
            ...
            <main class="lg:col-span-2 space-y-6">
                ...
                <div id="profile-panel-posts" data-profile-panel="posts" class="space-y-6">
                    <script type="application/json" id="profile-posts-config">@json($profilePostsConfig)</script>
    ...
    @auth
        @if (auth()->id() !== $user->id)
            <script>
                document.getElementById('profile-follow-btn')?.addEventListener('click', async function () {
                    const btn = this;
                    const following = btn.dataset.following === '1';
                    const storeUrl = @json(route('users.follow.store', $user->username));
                    const destroyUrl = @json(route('users.follow.destroy', $user->username));
                    try {
                        const res = following
